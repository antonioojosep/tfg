...
export default function ProductBrowser({ onAddProduct, order, onSendOrder, onRemoveProduct }) {
  ...
  // New function to remove product by index
      ...
      {order && order.length > 0 && (
        <div className="mt-8">
          ...
          <ul className="mb-2">
            {order.map((prod, idx) => (
              <li key={idx} className="flex justify-between items-center border-b py-1 gap-2">
                ...
                <Button
                  className="bg-danger hover:bg-red-700 px-2 py-1 text-xs rounded-xl ml-2"
          ...
          <div className="font-semibold mb-2">
            Total: {order.reduce((sum, p) => sum + (Number(p.price) || 0), 0).toFixed(2)} €
          </div>
